# -*- coding: utf-8 -*-
"""Real execution for SCANNED BCTC:
PDFium -> Qwen Vision REAL CALL -> GLM-5.2 REAL CALL -> token usage
-> verified financial facts -> section_d.net_revenue updated
-> customer.revenue_2025 NOT written -> ONE existing Python ratio engine
-> derived metrics.
"""
...
def main():
    print("=" * 80)
    print("2. SCANNED BCTC PIPELINE - REAL GREENNODE EXECUTION")
    print("=" * 80)

    pdf_path = os.path.join("tests", "fixtures", "bctc", "bctc_synthetic_scanned.pdf")
    ...
    assert ingestion_res.provider == "qwen_vision", "Expected Qwen Vision provider"

    # Step B: Display Qwen Vision OCR Telemetry
    telemetry = AIAssistantClient.get_telemetry(limit=10)
    ocr_tels = [t for t in telemetry if t.get("operation", "").startswith("ocr_page")]
    print("\n[B] Real Qwen Vision OCR Token Usage Telemetry:")
    for t in ocr_tels:
        print(f"  - Page Call: {t.get('operation')} | Model: {t.get('model')}")
        print(f"    In: {t.get('input_tokens')} | Out: {t.get('output_tokens')} | Total: {t.get('total_tokens')} | Latency: {t.get('latency_ms')} ms")

    # Step C: Real Call to GreenNode GLM-5.2
    print("\n[C] Real Call to GLM-5.2 (z-ai/glm-5.2-hackathon) from OCR text...")
    extractor = FinancialDocumentExtractor()
    extraction = extractor.extract(ingestion_res.tagged_text, ingestion_res.page_count)
    print("  [✓] GLM-5.2 returned valid financial extraction from OCR text!")

    # Step D: Display GLM-5.2 Telemetry
    telemetry_post = AIAssistantClient.get_telemetry(limit=5)
    glm_tel = next((t for t in telemetry_post if t.get("operation") == "financial_extraction"), None)
    if glm_tel:
        print("\n[D] Real GLM-5.2 Token Usage Telemetry:")
        print(f"  - Model: {glm_tel.get('model')}")
        print(f"  - Operation: {glm_tel.get('operation')}")
        print(f"  - Input Tokens: {glm_tel.get('input_tokens')}")
        print(f"  - Output Tokens: {glm_tel.get('output_tokens')}")
        print(f"  - Total Tokens: {glm_tel.get('total_tokens')}")
        print(f"  - Latency: {glm_tel.get('latency_ms')} ms")

    # Step E: Verified Financial Facts & Grounding Audit
    print("\n[E] Verified Financial Facts from Scanned BCTC:")
    total_audited = 0
    total_errors = 0
    ...
    # Step F: Canonical Mapping & RM Confirm
    print("\n[F] Canonical Case Data Binding & RM Confirm:")
    target_case_id = "CASE_VAN_XUAN_SCANNED"
    ...
    source_meta = MappingSourceMetadata(
        source_document="bctc_synthetic_scanned.pdf",
    ...
    preview_id = "preview_scanned_live"
    preview_record = FinancialPreviewRecord(
        preview_id=preview_id,
        case_id=target_case_id,
        extraction=extraction,
        source_filename="bctc_synthetic_scanned.pdf",
    ...
    print("\n[G] Checking Canonical Contract (ONE FACT = ONE CANONICAL PATH):")
    assert "net_revenue" in new_section_d, "section_d.net_revenue must be present"
    print("  [✓] section_d.net_revenue updated.")
    ...
    print(f"  [✓] customer.revenue_2025 NOT written (retains original: {cust_rev}).")
    ...
    # Step H: ONE Existing Python Ratio Engine & Auditable Lineage
    print("\n" + "=" * 80)
    ...
    print("[✓] SCANNED BCTC PIPELINE VERIFIED SUCCESSFULLY!")
    return 0
